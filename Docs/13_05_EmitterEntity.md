...
`EmitterEntity` — это компонент, управляющий источником частиц в мире. Он создаёт и уничтожает экземпляр частиц в зависимости от состояния `IsEmitting`.

Два режима работы:
- **Toggle** — нажал клавишу → включил, нажал ещё раз → выключил
- **Hold** — держишь клавишу → частицы идут, отпустил → выключились
