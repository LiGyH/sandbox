# Этап 14_06 — SpawnerWeapon

## Что делает этот компонент

**SpawnerWeapon** — это оружие, которое позволяет размещать объекты в мире. Представьте себе **строительный кран**: он может держать любой груз (пропы, сущности, дубликаты) и аккуратно устанавливать его именно туда, куда указывает игрок.

Это оружие принимает любой объект, реализующий интерфейс `ISpawner`, и берёт на себя прицеливание, предварительный просмотр, вращение и размещение.
...
- `SpawnerData` — строка, синхронизируемая с хоста на всех клиентов (`[Sync( SyncFlags.FromHost )]`).
- При изменении `SpawnerData` автоматически вызывается `OnPayloadDataChanged`, который пересоздаёт объект `ISpawner` на каждом клиенте.
- Формат строки: `тип:данные` (например, `"prop:models/chair.vmdl"` или `"dupe:{...json...}"`).

### Сериализация и десериализация спаунеров

- `SerializeSpawner` — превращает `ISpawner` в строку формата `тип:данные` в зависимости от конкретного типа (`PropSpawner`, `EntitySpawner`, `DuplicatorSpawner`).
- `DeserializeSpawner` — обратная операция: разбивает строку по первому символу `:` и создаёт соответствующий объект.

### Управление игроком (`OnControl`)

- Обновляет экран модели оружия и анимацию катушки.
- Если нажата клавиша `use` — включается режим вращения: движения мыши поворачивают предмет, а не камеру.
- Если одновременно зажата клавиша `run` — вращение «привязывается» к сетке с шагом 45°.
- Левая кнопка мыши (`attack1`) — размещает объект, если цель валидна и спаунер готов.
- Правая кнопка мыши (`attack2`) — убирает оружие из инвентаря.

### Предварительный просмотр (`DrawPreview`)

- Вызывается в `OnUpdate` на **всех клиентах**, чтобы каждый игрок видел, что размещает другой.
- Для чужих превью (`IsProxy`) используется другой материал (`_previewMaterialInvalid`), чтобы отличить свой предмет от чужого.

### Вычисление позиции размещения (`GetSpawnTransform`)

- Луч прицеливания бросается из глаз игрока на расстояние 4096 единиц.
- Нормаль поверхности используется как «вверх», чтобы объект правильно ориентировался на наклонных поверхностях.
- Объект смещается вверх по границам (`Bounds`), чтобы не проваливался в поверхность.
- К вращению добавляется `_rotationOffset` — то, что игрок «накрутил» в режиме вращения.

### Создание объектов на сервере (`DoSpawn`)

- Помечен `[Rpc.Host]` — выполняется только на хосте.
- Находит игрока по `Rpc.Caller` для безопасности.
- Вызывает `Spawner.Spawn`, получает список созданных объектов.
- Если объекты созданы, регистрирует их в системе отмены (`Undo`).

### Отрисовка прицела (`DrawHud`)

- Без спаунера рисует маленький полупрозрачный круг.
- Со спаунером рисует цветной индикатор: белый — можно разместить, красноватый — нельзя.

### Интерфейс `IToolInfo`

- Предоставляет информацию для отображения в HUD: название инструмента, описание текущего действия, подписи для кнопок.
