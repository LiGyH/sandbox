# 🧰 ToolMode.Helpers — Вспомогательные методы инструментов

## Что мы делаем?

Создаём файл `ToolMode.Helpers.cs` — partial-часть класса `ToolMode`, содержащая вспомогательные структуры и методы: `SelectionPoint` для трассировки, `TraceSelect` для поиска целей, `GetEasyModePlacement` для размещения объектов и `ApplyPhysicsProperties` для применения физических свойств.

## Зачем это нужно?

Эти утилиты используются всеми инструментами:
- `SelectionPoint` — универсальная структура для описания точки попадания с привязкой к объекту
- `TraceSelect` — трассировка луча от глаз игрока с фильтрацией по тегам и проверкой прав через `IToolgunEvent`
- `GetEasyModePlacement` — вычисление позиции размещения «лицом к лицу» двух объектов
- `ApplyPhysicsProperties` — применение физических свойств из модели к Rigidbody

## Как это работает внутри движка?

- `SelectionPoint` хранит `GameObject` и `LocalTransform` (локальная позиция/нормаль относительно объекта)
- `WorldPosition()` и `WorldTransform()` преобразуют локальные координаты в мировые
- `IsWorld` и `IsPlayer` проверяют теги
- `TraceSelect` — выполняет `Scene.Trace.Ray` от глаз игрока, фильтрует по `TraceIgnoreTags`, проверяет `IToolgunEvent.SelectEvent`
- `GetEasyModePlacement` — вычисляет Transform для размещения объекта A на точке B так, чтобы поверхности совпали
- `ApplyPhysicsProperties` — читает физические параметры (масса, damping, центр масс) из `Model.Physics` и применяет к `Rigidbody`
...
- Файл `Code/Weapons/ToolGun/ToolMode.Helpers.cs` создан как partial-класс `ToolMode`
- Структура `SelectionPoint` с методами `IsValid`, `WorldPosition`, `WorldTransform`, `IsWorld`, `IsPlayer`
- `TraceSelect` выполняет трассировку с фильтрацией тегов и проверкой `IToolgunEvent`
- `GetEasyModePlacement` вычисляет transform для размещения объектов
- `ApplyPhysicsProperties` применяет физические свойства из модели
