...
Создаём файл `WeldTool.cs` — инструмент, который **жёстко соединяет** два объекта (фиксированное соединение). Это самый сложный инструмент-соединение, потому что у него есть:
- **Easy Mode** — автоматическое позиционирование + вращение
- **3 стадии** (выбор первой точки → выбор второй → вращение)
- Привязка к сетке (snap grid)

## Зачем это нужно?

Weld — основной строительный инструмент:
- Приварить колесо к машине
- Соединить стены дома
- Создать сложные конструкции
...
|----------|-----|-------------|----------|
| `EasyMode` | `bool` | `true` | Автоматическое позиционирование + вращение |
| `Rigid` | `bool` | `false` | Если true — соединение полностью жёсткое (нет упругости) |

### 3 стадии (Easy Mode)

```
Stage 0 → Выбор первого объекта (ЛКМ)
  └── Запоминает Point1, переходит в Stage 1

Stage 1 → Выбор второго объекта (ЛКМ)
  └── Показывает ghost-preview первого объекта на позиции второго
  └── Переходит в Stage 2

Stage 2 → Вращение (мышью)
  └── Мышь вращает объект вокруг точки контакта
  └── Shift = привязка к 15° сетке
  └── ЛКМ = подтвердить, ПКМ = отмена
```
...
На стадии 2 мышь **не двигает камеру** — вместо этого она вращает объект.
...
Рисуется полупрозрачная копия объекта и всех связанных с ним объектов (`LinkedGameObjectBuilder`).

### FixedJoint (жёсткое соединение)
...
joint.AngularFrequency = Rigid ? 0 : 10;  // 0 = абсолютно жёстко
joint.LinearFrequency = Rigid ? 0 : 10;
```

### Snap Grid + Weld

В Easy Mode сварка использует snap grid для точного позиционирования. Зажатие E привязывает прицел к ближайшему углу сетки.
...
1. EasyMode: объект перемещается к точке и показывает ghost-preview
2. Stage 2: мышь вращает объект, Shift = привязка к 15°
3. ЛКМ подтверждает, ПКМ отменяет
4. R удаляет сварку с объекта
5. Rigid = true → абсолютно жёсткое соединение

---

➡️ Следующий шаг: [10_05 — BallSocket (шарнир)](10_05_BallSocket.md)
