...
Создаём файл `Weld.cs` — инструмент, который **жёстко соединяет** два объекта (фиксированное соединение). Это самый сложный инструмент-соединение, потому что у него есть:
