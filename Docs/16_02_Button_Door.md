# 16.02 — Кнопка (Button) 🔘 (УСТАРЕВШИЙ ДОКУМЕНТ ⚠️)

> ## ⚠️ Этот документ устарел
>
> В актуальной версии sandbox **компонент `Button` (`Code/Map/Button.cs`) удалён** из исходного кода вместе с парным [`Door`](16_02b_Door.md) — Facepunch перенёс обработку дверей и кнопок на уровень самого движка / маппинга. Сейчас в `Code/Map/` остался только базовый абстрактный [`BaseToggle`](16_01_BaseToggle.md), а также `FuncMover`, `MapPlayerSpawner` и `Trigger*`-компоненты.
>
> **Что использовать вместо этого:**
>
> 1. Для собственных кнопок наследуйся от [`BaseToggle`](16_01_BaseToggle.md) и реализуй интерфейс движка `Component.IPressable` — это та же модель, что была у `Button`.
> 2. Для готовых дверей/кнопок на карте используй движковые префабы и компоненты Facepunch — см. [официальную документацию](https://github.com/Facepunch/sbox-docs/) (раздел Components → Pressable / Animated Doors).
> 3. Если в твоей карте/режиме всё ещё нужна старая `Button` — скопируй приведённый ниже код в `Code/Map/Button.cs` своего проекта; он работоспособен, но больше не сопровождается апстримом.
>
...
Переходи к **[16.02b — Дверь (Door)](16_02b_Door.md)** — дверь как IPressable-объект.
