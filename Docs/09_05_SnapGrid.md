# 📐 ToolMode.SnapGrid.cs и SnapGrid.cs — Система привязки к сетке

## Что мы делаем?

Создаём **два файла**:
1. `ToolMode.SnapGrid.cs` — расширение `partial class ToolMode` для интеграции сетки привязки
2. `SnapGrid.cs` — самостоятельный класс, который рисует сетку на поверхности объектов

## Зачем это нужно?

Когда игрок использует инструмент Weld (сварку) или другой инструмент с точным позиционированием, ему нужно видеть **сетку** на поверхности объекта, чтобы точно выбрать точку. Snap Grid:

- Рисует полупрозрачную сетку на поверхности наведённого объекта
- Подсвечивает ближайшую линию/угол сетки
- Позволяет «примагнитить» прицел к углу сетки (зажать E)
- Автоматически подстраивает размер ячеек под размер объекта
- Ориентирует сетку по объекту (учитывает поворот)
...
```
ToolMode (partial — SnapGrid)
  ├── UseSnapGrid         — переопределить в true, чтобы включить
  ├── SnapGrid            — ссылка на экземпляр SnapGrid
  ├── OnControl()         — обновляет сетку каждый кадр
  ├── OnCameraMove()      — позволяет зажать E для lock-on к углу
  ├── ShouldDisplaySnapGrid() — фильтр (не на мире/игроках)
  └── DisableSnapGrid()   — уничтожение при выключении

SnapGrid (отдельный класс)
  ├── CellSize            — размер ячейки (по умолчанию 4 юнита)
  ├── MaskRadius          — радиус видимости сетки (64 юнита)
  ├── Update()            — обновление каждый кадр
  ├── ComputeSnap()       — вычисление ближайшего угла
  ├── Hide()              — скрыть наложение
  ├── Destroy()           — уничтожить SceneObject
  └── LastSnapWorldPos    — последняя привязанная позиция
```

### Как работает snap

`ComputeSnap()` принимает позицию прицела и вычисляет:
1. Проецирует позицию на плоскость сетки (U/V координаты)
2. Находит ближайший узел сетки (округление до ячейки)
3. Если курсор **близко к углу** (< 35% ячейки) — привязывает к углу
4. Если ближе к одной оси — привязывает только по этой оси

### OBB (Oriented Bounding Box)

Сетка должна совпадать с объектом. Для этого `GetObjectOBB()`:
1. Сначала пробует получить bounds из `ModelRenderer` (точные)
2. Если нет — из `SkinnedModelRenderer`
3. Если нет — из `Collider` (менее точные, AABB)
4. Ориентирует сетку по rotation объекта
...
После создания обоих файлов класс `ToolMode` теперь полностью собран из 5 partial-файлов:
1. `ToolMode.cs` — основной
2. `ToolMode.Cookies.cs` — сохранение
3. `ToolMode.Effects.cs` — эффекты
4. `ToolMode.Helpers.cs` — трассировка и хелперы
5. `ToolMode.SnapGrid.cs` — привязка к сетке

Для работы сетки также нужен шейдер `shaders/snap_grid.shader` (он должен быть в Assets проекта).

---

➡️ Следующий шаг: [09_06 — Toolgun.cs](09_06_Toolgun.md)
