...
Создаём **FuncMover** — компонент для движущихся и вращающихся объектов на карте (платформы, лифты, вентиляторы).

## Возможности

| Функция | Описание |
|---------|----------|
| **LinearMove** | Движение по прямой линии от стартовой позиции к целевой и обратно |
| **RotateMove** | Постоянное вращение вокруг своей оси |
| **Loop** | Автоматическое возвращение в начало с паузой |
| **Toggle** | Включение/отключение через консольную команду или Action Graph |

## Создай файл

Путь: `Code/Map/FuncMover.cs`
...
`DrawGizmos` рисует:
- **Зелёная сфера** — стартовая позиция
- **Красная сфера** — конечная позиция
- **Белая линия** — путь движения
- **Голубая сфера** — анимированный предпросмотр положения (синусоида)

## Результат

- Платформы, лифты, вращающиеся объекты для карт
- Гизмо для визуализации пути в редакторе
- Управление через Rpc (мультиплеер)
