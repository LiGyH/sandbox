# ✨ Physgun.Effects — Визуальные эффекты и экран физической пушки
...
Создаём файл `Physgun.Effects.cs` — partial-часть класса `Physgun`, отвечающая за визуальные эффекты луча, частицы на точке захвата, графики на экране оружия и отрисовку содержимого экрана.

## Зачем это нужно?

Визуальная обратная связь делает инструмент понятным и приятным в использовании:
- Луч (beam) из дула к точке захвата с пружинной анимацией
- Эффекты на точке попадания (EndPoint) и на захваченном объекте (Grab)
- Анимированный экран на модели оружия с bar-индикатором мощности и графиками каналов
- Текстура графика обновляется динамически на основе состояния (захват, притягивание, вращение)

## Как это работает внутри движка?

- `UpdateBeam` — обновляет 4 точки `LineRenderer` с пружинной интерполяцией среднего узла через `Vector3.SpringDamped`
- `CloseBeam` — скрывает луч и уничтожает эффекты при отпускании
- `DrawScreenContent` — рисует на экране viewmodel: bar мощности, контур, процент и метки каналов через `HudPainter`
- `UpdateScreenGraph` — собирает 3 канала данных (захват/притягивание/вращение) в кольцевой буфер и записывает пиксели в 1D-текстуру `_graphTexture`
- Текстура и параметры графика передаются на `SceneObject` через `Attributes.Set` для шейдера экрана
...
- Файл `Code/Weapons/PhysGun/Physgun.Effects.cs` создан как partial-класс `Physgun : ScreenWeapon`
- Свойства луча, эффектов и параметров экрана объявлены через `[Property]`
- `UpdateBeam` управляет 4 точками `LineRenderer` с пружинной интерполяцией
- `DrawScreenContent` рисует bar-индикатор и текстовые метки на экране оружия
- `UpdateScreenGraph` генерирует 3 канала графика и передаёт текстуру в шейдер
...
Переходи к **[08.03 — 🎯 Physgun.Hud — HUD-прицел физической пушки](08_03_Physgun_Hud.md)**.
