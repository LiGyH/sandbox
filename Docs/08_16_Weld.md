...
Создаём инструмент `Weld` — режим Tool Gun, который жёстко скрепляет два объекта с помощью `FixedJoint`. Поддерживает «Easy Mode» с визуальным превью и ручной подстройкой угла поворота перед финальной сваркой.

## Зачем это нужно?

Сварка — один из основных инструментов в песочнице. Она позволяет:
- Жёстко соединять объекты в конструкции
- В режиме Easy Mode — автоматически выравнивать объект по поверхности и вращать перед приваркой
- Использовать привязку к сетке (Snap Grid) для точного позиционирования
- Выбирать между жёсткой (`Rigid`) и упругой связью

## Как это работает внутри движка?

- **`EasyMode`** — режим с визуальным превью и третьей стадией (вращение).
- **`Stage == 2`** (только Easy Mode) — стадия вращения: мышь управляет углом, Shift привязывает к 15°, `attack1` подтверждает, `attack2` отменяет.
- **`SnapSelectionPoint()`** — корректирует точку выбора, привязывая её к ближайшей позиции на сетке.
- **`GetEasyModePlacement()` / `GetRotatedPlacement()`** — вычисляют трансформ объекта при размещении с учётом вращения.
- **`DrawEasyModePreview()`** — рисует полупрозрачный призрак объекта и всех связанных с ним объектов в позиции размещения.
- **`CreateConstraint()`** — создаёт два дочерних `GameObject` с `FixedJoint`, `ConstraintCleanup` и регистрирует Undo.
- **`FindConstraints()`** — ищет `FixedJoint` для удаления при нажатии Reload.
...
- Класс `Weld` наследуется от `BaseConstraintToolMode`.
- Три стадии: выбор источника → выбор цели → (Easy Mode) вращение.
- `FixedJoint` создаёт жёсткую связь между двумя объектами.
- `Rigid` переключает между упругой (frequency = 10) и абсолютно жёсткой (frequency = 0) связью.
- Easy Mode показывает превью, поддерживает привязку к сетке и вращение с шагом 15°.
- `ConstraintCleanup` обеспечивает удаление связанных объектов при разрушении.
- Undo регистрируется для возможности отмены действия.
