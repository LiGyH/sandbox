...
- Цвет вычисляется как одна `Color.Lerp(PhysTint, GravTint, t)`, но `t` пропускается через крутую ease-in-out функцию (`8·x⁴` и зеркальное `1 - 8·(1-x)⁴`), чтобы переход «проскакивал» белую середину быстро и не было затяжного засветления
...
- Реализована плавная интерполяция цвета между режимами Phys/Grav через `_tintFrac`, с крутой ease-in-out функцией, которая ускоряет проход через белую середину
...
- `_tintFrac` плавно интерполируется от 0 (Phys) до 1 (Grav) через `MathX.Approach`
- Цвет — это `Color.Lerp(PhysTint, GravTint, t)`, где `t` — крутая ease-in-out (`8·x⁴` / `1 - 8·(1-x)⁴`). Промежуточный белый теперь не задерживается посередине — переход «проскакивает» его
...
- Плавная интерполяция цвета `PhysTint → GravTint` с крутым ease-in-out, который быстро проскакивает белую середину
...
Переходи к **[08.05 — Компонент: Событие физ-пушки (IPhysgunEvent) 🔫](08_05_IPhysgunEvent.md)**.
