...
> ⚠️ **Удалено в обновлении движка — историческая справка.** Файл `Code/Npcs/Speech/SubtitleExtension.cs` был удалён из исходного кода sandbox, а вместе с ним и вызов `SubtitleExtension.FindForResourceOrDefault(...)` в `SpeechLayer`. Теперь субтитры задаются **только** явным параметром `subtitle` при вызове речи (см. `22_06`), а механизм автоматического поиска `.ssub`-файла по звуку больше не используется. Кроме того, `SpeechLayer.DrawSpeech()` теперь не показывает субтитры **сквозь стены** (трассировка луча по тегу `world`). Раздел ниже сохранён как историческая справка по `ResourceExtension<TResource, TSelf>`.
...
Фаза завершена. Переходи к **[23.01 — Интерфейс событий сохранения (ISaveEvents)](23_01_ISaveEvents.md)** — начало следующей фазы.
