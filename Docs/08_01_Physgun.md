...
- `AimTransform` / `CanAim` — источник прицеливания: глаза игрока, когда оружие в руках; `MuzzleTransform` (при желании с поворотом глаз сидящего игрока), когда Physgun стоит самостоятельно
...
|---|---|
| `CanAim` | `[Property, ClientEditable, Sync]` — разрешает «подхватывать» поворот глаз сидящего игрока (`ClientInput.Current.EyeTransform.Rotation`) для наведения в standalone-режиме. |
...
| `OnControl()` (без параметра) | Seat/standalone-версия управления: `ShootInput` захватывает/запускает объект, `SecondaryInput` притягивает/отменяет. Пересобирает те же стадии `GrabState`, но берёт `AimTransform` вместо `Owner.EyeTransform`. |
