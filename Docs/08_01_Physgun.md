...
- `OnControl` — основной метод управления: обрабатывает ввод (attack1, attack2, use, reload, mousewheel) и переключает режимы захвата/притягивания/вращения
- `OnFixedUpdate` — физический шаг: создаёт `ControlJoint` между кинематическим телом и захваченным объектом, обновляет целевую позицию
