# 🔫 Physgun — Основная логика физической пушки
...
Создаём файл `Physgun.cs` — основной класс физической пушки (Physgun), которая позволяет игроку захватывать, перемещать, вращать, замораживать и запускать физические объекты в мире.

## Зачем это нужно?

Physgun — один из ключевых инструментов в песочнице. Он позволяет:
- Захватывать объекты лучом и удерживать их на расстоянии
- Притягивать объекты к игроку (Pull)
- Вращать захваченные объекты с привязкой к сетке углов
- Замораживать и размораживать физические тела
- Запускать объекты с силой (Launch)
- Управлять дистанцией захвата колёсиком мыши

## Как это работает внутри движка?

- `GrabState` — структура, хранящая состояние захвата: объект, смещение, нормаль, дистанцию и вращение
- `OnControl` — основной метод управления: обрабатывает ввод (attack1, attack2, use, reload, mousewheel) и переключает режимы захвата/притягивания/вращения. Дополнительно есть перегрузка `OnControl()` **без игрока** — для режима сиденья / standalone (см. ниже)
- `AimTransform` / `CanAim` — теперь полностью построены поверх **унифицированного `AimRay`** из `BaseCarryable` (см. [06.01](06_01_BaseCarryable.md)). Свойство `IsTargetedAim => CanAim` сообщает базовому классу, что в сидении нужно прицеливаться по камере игрока, а не по дулу
- `_lastAimTransform` / `CurrentAimTransform` — кэш последнего `AimTransform`, чтобы эффекты и `OnFixedUpdate` корректно работали даже после того, как владелец ушёл
- `OnFixedUpdate` — физический шаг: создаёт `ControlJoint` между кинематическим телом и захваченным объектом, обновляет целевую позицию (использует `CurrentAimTransform`, а не напрямую `Owner.EyeTransform`)
- `FindGrabbedBody` — трассировка луча для поиска объекта под прицелом, проверка `IPhysgunEvent`
- `Freeze/Unfreeze/UnfreezeAll` — RPC-методы для управления замораживанием через `MotionEnabled`
- `Launch` — RPC-метод для запуска объекта с импульсом
- Вращение с привязкой к сетке углов (`SnapToGrid`) при зажатых клавишах run/walk
...
- Файл `Code/Weapons/PhysGun/Physgun.cs` создан и содержит класс `Physgun` с partial-модификатором
- Структура `GrabState` описывает состояние захвата с синхронизацией через `[Sync]`
- Метод `OnControl` обрабатывает весь ввод: захват, притягивание, вращение, замораживание
- Перегрузка `OnControl()` без параметра работает в режиме сиденья/standalone
- `OnFixedUpdate` управляет физическим джоинтом между кинематическим телом и объектом
- RPC-методы `Freeze`, `Unfreeze`, `UnfreezeAll`, `Launch` обеспечивают сетевую синхронизацию

## Режим standalone / seat 🪑

Начиная с актуального апстрима, Physgun умеет работать **без игрока в руках** — как часть сиденья или турели. Это открывает два новых пути использования: приваренный к машине манипулятор или пушка, управляемая игроком через `ClientInput`.

| Поле / метод | Что делает |
|---|---|
| `CanAim` | `[Property, ClientEditable, Sync]` — разрешает использовать камеру сидящего игрока для прицеливания. Транслируется как `override IsTargetedAim => CanAim` в `BaseCarryable`, поэтому `AimRay` сам выберет камеру вместо дула. |
| `AimTransform` (getter) | Сейчас тонкая обёртка вокруг `BaseCarryable.AimRay`: возвращает `Transform` с позицией и поворотом луча. Внутри уже учтены три случая (1-е лицо / 3-е лицо / сидение с `IsTargetedAim`). |
...
### Как подключить standalone-мод

1. Поставь `Physgun` на GameObject в сцене или префабе турели.
2. Задай ему `MuzzleGameObject` у `BaseCarryable` (см. [06.01](06_01_BaseCarryable.md)) или добавь `WeaponModel` с корректным muzzle-трансформом.
3. В твоей системе сиденья/`ClientInput` вызывай `physgun.OnControl()` каждый кадр, пока игрок управляет оружием.
4. Оставь `CanAim = true`, чтобы турель «смотрела» за глазами игрока; выключи, чтобы использовать только поворот самого `MuzzleTransform`.
...
Переходи к **[08.02 — ✨ Physgun.Effects — Визуальные эффекты и экран физической пушки](08_02_Physgun_Effects.md)**.
