# 🛠️ ToolMode — Базовый абстрактный класс режима инструмента

## Что мы делаем?

Создаём файл `ToolMode.cs` — абстрактный базовый класс для всех режимов инструментальной пушки. Определяет общий интерфейс, жизненный цикл, отрисовку экрана/HUD и систему достижений.

## Зачем это нужно?

`ToolMode` — точка расширения для всех инструментов (Weld, Rope, Wheel, Balloon и т.д.):
- Определяет интерфейс `IToolInfo` с описаниями действий
- Управляет загрузкой/сохранением настроек через cookies
- Предоставляет общий HUD-прицел с двумя состояниями (valid/invalid)
- Отрисовывает экран viewmodel с названием инструмента
- Отслеживает статистику контрапций для системы достижений
- Поддерживает поглощение ввода мыши и фильтрацию трассировки по тегам

## Как это работает внутри движка?

- `Toolgun` и `Player` — быстрый доступ к родительским компонентам
- `IsValidState` — флаг, устанавливаемый в `OnControl` для индикации валидности
- `AbsorbMouseInput` — виртуальное свойство для поглощения мыши (используется Weld для вращения)
- `Name`, `Description`, `PrimaryAction`, `SecondaryAction`, `ReloadAction` — виртуальные свойства для UI
- `TraceIgnoreTags` — фильтрация игроков при трассировке
- `OnEnabled/OnDisabled` — загрузка/сохранение cookies и отключение snap grid
- `DrawScreen` — рисует название и иконку инструмента на экране viewmodel
- `DrawHud` — рисует прицел (белый/красный) в зависимости от `IsValidState`
- `CheckContraptionStats` — RPC для владельца, собирает статистику контрапции через `LinkedGameObjectBuilder`
...
- Файл `Code/Weapons/ToolGun/ToolMode.cs` создан как абстрактный partial-класс `ToolMode : Component, IToolInfo`
- Виртуальные свойства `Name`, `Description`, `PrimaryAction` и др. определены
- `OnEnabled/OnDisabled` управляют cookies и snap grid
- `DrawScreen` и `DrawHud` предоставляют базовую отрисовку
- `CheckContraptionStats` собирает статистику контрапций через RPC
