# 🔲 SnapGrid — Система визуальной сетки привязки

## Что мы делаем?

Создаём файл `SnapGrid.cs` — класс, управляющий отображением сетки привязки в мировом пространстве на поверхности объектов. Включает вычисление OBB, проекцию на плоскость, рендеринг через `SceneDynamicObject` и алгоритм привязки к ближайшей линии/углу.

## Зачем это нужно?

Snap grid обеспечивает:
- Точное позиционирование при сварке, размещении и других операциях
- Визуальную сетку, автоматически ориентированную по граням объекта
- Привязку к ближайшей линии или углу с двухосевой логикой
- Масштабируемый размер ячеек, адаптирующийся к размеру объекта
- Маскирование с затуханием от точки прицеливания

## Как это работает внутри движка?

- `SnapGridSceneObject` — внутренний класс, наследующий `SceneDynamicObject`, рисует quad на плоскости грани
- `ComputeSnap` — статический метод: вычисляет ближайший узел сетки (cx, cy), определяет привязку по одной или двум осям в зависимости от близости к углу
- `Update` — основной метод: определяет плоскость грани через OBB объекта, вычисляет оси Right/Up, проецирует центр на плоскость, масштабирует ячейки, передаёт параметры в шейдер
- `GetObjectOBB` — вычисляет ориентированный ограничивающий параллелепипед из модели или коллайдеров
- `ProjectOntoPlane` — проекция точки на плоскость
- `ProjectedHalfExtents` — вычисление полуразмеров OBB, спроецированных на оси грани
- Параметры шейдера: `GridOrigin`, `GridRight`, `GridUp`, `AimPoint`, `MaskRadius`, `CellSize`, `SnapCornerX/Y`, `SnapAxisX/Y`
...
- Файл `Code/Weapons/ToolGun/SnapGrid.cs` создан с классом `SnapGrid`
- `SnapGridSceneObject` наследует `SceneDynamicObject` и рисует quad
- `ComputeSnap` вычисляет ближайший узел с двухосевой логикой
- `Update` определяет плоскость через OBB, передаёт параметры в шейдер
- `GetObjectOBB` поддерживает модели и коллайдеры как fallback
- `ProjectOntoPlane` и `ProjectedHalfExtents` — математические утилиты
